...
export const askLeshiy = async ({ text, files = [], history = [], isSystemTask = false }) => {
    ...
    if (isSystemTask) {
        ...
        const systemInstruction = "Ты — помощник, который придумывает краткие и емкие названия для чатов (максимум 5-6 слов) на основе контекста. Игнорируй любые технические данные о подключении облачных дисков, статусах системы или 'Хранилке'. Используй только суть вопроса пользователя или твоего содержательного ответа. Отвечай ТОЛЬКО названием, без кавычек и лишних слов.";
